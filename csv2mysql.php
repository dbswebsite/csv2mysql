...
class csv2mysql
{
	...
	protected $csv 		= null;		// csv file name
	protected $db 			= null;		// database name
	protected $table_name	= null;		// table name
	protected $db_host	 	= 'localhost';
	protected $db_user 		= 'root';
	protected $db_password	= '';

	protected $debug 			= true;
	protected $create 			= true;	// dynamically create table structure or not. TODO: not tested if this is FALSE :/
	...
	protected $custom_sql		= null;	// custom sql to run after the table has been built.
	...
	*/
	function __construct() {
		...
		if ( ! $this->table_name ) $this->table_name = str_replace( '.csv', '', basename( $this->csv ) );
	...
	*/
	protected function create_table()
	{
		...
		// Extract first (header) record only, use for mysql column names.
		...
		for( $i=0;$i<count( $this->columns ); $i++ ) {
			...
			$this->columns[$i] = str_replace( ' ', '_', $this->columns[$i] );
			$this->columns[$i] = str_replace( array( "'", "/",'\\',".",'"','?','$','-','*','`','+', ','), '', $this->columns[$i] );
